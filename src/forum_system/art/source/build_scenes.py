...
"""
import bpy, math, random, sys, time, os, hashlib
...
water=material('water',(.016,.033,.036),(.038,.065,.068),3,.09,0)
damp=material('damp paving sheen',(.035,.049,.047),(.08,.11,.104),5,.24,.08)
cloth=material('undyed hanging cloth',(.13,.14,.12),(.5,.47,.35),40)
def box(name,p,s,mat=wood,bev=.03,rot=0):
 # Direct meshes avoid thousands of slow selection/depsgraph operator updates.
 x,y,z=(v/2 for v in s)
 verts=[(-x,-y,-z),(-x,-y,z),(-x,y,-z),(-x,y,z),(x,-y,-z),(x,-y,z),(x,y,-z),(x,y,z)]
 faces=[(2,6,4,0),(5,7,3,1),(4,5,1,0),(3,7,6,2),(1,3,2,0),(6,7,5,4)]
 mesh=bpy.data.meshes.new(name);mesh.from_pydata(verts,[],faces);mesh.update()
 o=bpy.data.objects.new(name,mesh);bpy.context.collection.objects.link(o);o.location=p;o.rotation_euler.z=rot
...
def cyl(name,p,r,d,mat=wood,vertices=32):
 verts=[(r*math.cos(i*math.tau/vertices),r*math.sin(i*math.tau/vertices),z) for z in [-d/2,d/2] for i in range(vertices)]
 ...
 mesh=bpy.data.meshes.new(name);mesh.from_pydata(verts,[],faces);mesh.update()
 o=bpy.data.objects.new(name,mesh);bpy.context.collection.objects.link(o);o.location=p;mesh.materials.append(mat)
 mod=o.modifiers.new('edge','BEVEL');mod.width=.018;mod.segments=2;o.modifiers.new('normals','WEIGHTED_NORMAL');return o
...
def hanging_panel(name,x,y,z,w,h,mat):
 verts=[];faces=[];cols=32;rows=8
 for j in range(rows+1):
  for i in range(cols+1):
   u=i/cols;v=j/rows
   verts.append((x+(u-.5)*w,y+.042*math.sin(u*math.pi*12),z+(v-.5)*h+.015*(1-v)*math.cos(u*math.pi*6)))
 for j in range(rows):
  for i in range(cols):
 ...
 mod=o.modifiers.new('fabric thickness','SOLIDIFY');mod.thickness=.004
 return o
def render(name):
 started=time.monotonic();print('BUILD',name,flush=True)
 random.seed(int.from_bytes(hashlib.sha256(name.encode()).digest()[:4],'big'))
 for obj in list(bpy.data.objects):bpy.data.objects.remove(obj,do_unlink=True)
 for mesh in list(bpy.data.meshes):
  if mesh.users==0:bpy.data.meshes.remove(mesh)

 # Related rooms share a palette; unrelated cases should not look like the same room.
 palettes={
  'snow':((.06,.073,.084),(.34,.36,.35)),
  'shadowplay':((.025,.045,.052),(.16,.22,.22)),
  'bridal':((.065,.043,.035),(.29,.215,.155)),
  'mirror':((.03,.054,.046),(.20,.27,.20)),
  'wardrobe':((.03,.054,.046),(.20,.27,.20)),
  'well':((.025,.045,.052),(.20,.235,.24)),
  'caravan':((.035,.045,.04),(.205,.21,.16)),
 }
 a,b=palettes.get(name,((.04,.055,.054),(.29,.28,.22)))
 ramp=next(n for n in plaster.node_tree.nodes if n.type=='VALTORGB').color_ramp
 ramp.elements[0].color=(*a,1);ramp.elements[1].color=(*b,1)
 ...
 if name in ['lamp','wall']:
  ...
  box('sedan roof',(-1.8,.9,1.94),(1.5,1.42,.13),red);hanging_panel('sedan curtain',-1.8,1.5,1.12,1.05,1.4,red)
  ...
  frost=material('frosted winter glass',(.12,.21,.27),(.55,.66,.70),19,.6)
  for x in [-3.5,3.5]:
   box('frost behind lattice',(x,3.414,2),(1.14,.012,1.44),frost,.002)
   box('snow on outside sill',(x,3.3,1.22),(1.3,.2,.08),frost)
   glow((x,3.08,2.2),24,(.36,.60,1),.8)
 elif name=='shadowplay':
  box('stage',(0,1.6,.3),(5,2,.6));box('paper theatre screen',(0,2.6,2.1),(3.1,.04,1.95),cloth)
  for x in [-1.9,1.9]:hanging_panel('red stage curtain',x,2.1,2.1,.65,2.25,red)
 ...
 moss=material('lower wall patina',(.045,.055,.044),(.11,.115,.08),11,.95)
 for i in range(66):
  x=random.uniform(-4.15,4.15);z=random.uniform(.06,.5)
  w=random.uniform(.025,.15);h=random.uniform(.08,.33)
  points=[(x-w,3.475,.04),(x-w*.7,3.475,z+h*.5),(x-w*.25,3.475,z+h),(x+w*.3,3.475,z+h*.65),(x+w,3.475,z),(x+w*.8,3.475,.04)]
  mesh=bpy.data.meshes.new('plaster water stain');mesh.from_pydata(points,[],[tuple(range(len(points)))]);mesh.update()
  o=bpy.data.objects.new('plaster water stain',mesh);bpy.context.collection.objects.link(o);mesh.materials.append(moss)
 for i in range(14):
 ...
 # Exterior wet patches; dry archives/lofts and winter rooms do not have indoor puddles.
 wet_count=10 if name in ['well','caravan','gate'] else 2 if name in ['lamp','wall','bridal'] else 0
 for i in range(wet_count):
  x=random.uniform(-3.5,3.5); y=random.uniform(-2.3,1.4 if wet_count>2 else -1.6)
  ...
  obj=bpy.data.objects.new('reflected eave light',mesh);bpy.context.collection.objects.link(obj);obj.data.materials.append(damp)
 ...
 d=bpy.data.lights.new('moon through courtyard','AREA');d.energy=240 if name=='snow' else 210 if name=='well' else 145 if name in ['bridal','shadowplay'] else 190;d.color=(.42,.66,1) if name=='snow' else (.32,.60,.69);d.shape='DISK';d.size=6;o=bpy.data.objects.new(d.name,d);bpy.context.collection.objects.link(o);o.location=(-2,-1,5)
 ...
 # Optional local acceleration; CPU fallback is always available. No saved global preferences.
 scene.cycles.device='CPU'
 if os.environ.get('WEIMING_RENDER_DEVICE')=='METAL':
  try:
  ...
  except Exception as e:print('Metal unavailable; using CPU:',e,flush=True)
 print('RENDER',name,scene.cycles.device,'build seconds',round(time.monotonic()-started,2),flush=True)
 ...
 # Keep the saved source portable; render to an absolute output only in memory.
 scene.render.filepath='//../../assets/scenes/'+f'{name}.jpg'
 ...
 scene.render.filepath=str(ROOT/'assets/scenes'/f'{name}.jpg')
 bpy.ops.render.render(write_still=True)
 print('DONE',name,'total seconds',round(time.monotonic()-started,2),flush=True)
