...
新增可玩本必须有独立的人物、时间线、伏笔、失败反馈和完整通关验证，不只是把一个参考条目改成 `playable`。详细流程见 [剧本库说明](cases/README.md)。浏览器测试命令见 [安装指南](docs/install_guide.md)，迁移结果见 [V4.1 验收](docs/validation.md)，本次视听更新见 [V5 说明](docs/immersion-v5.md)。
...
### V5 audiovisual polish

The story and progression remain unchanged. Twelve locally modeled scenes are rerendered at 1920×1200, with weathered materials, paper/cloth interface textures and restrained warm/cool lighting. Offline Web Audio supplies location-specific ambience and subtle interaction cues. Sound is off by default; master, ambience and effects have independent controls. Background tabs suspend audio, tapes duck the ambience, and reduced motion is respected. No third-party art, music service or runtime API is required.
